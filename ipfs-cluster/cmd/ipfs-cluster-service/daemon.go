...
import (
	...
	"github.com/ipfs-cluster/ipfs-cluster/informer/storage"
...
func createCluster(
	...
	if cfgMgr.IsLoadedFromJSON(config.Informer, cfgs.StorageInf.ConfigKey()) {
		logger.Debugf("setting storage informer")
		storageInf, err := storage.NewInformer(cfgs.StorageInf)
		checkErr("creating storage informer", err)
		informers = append(informers, storageInf)
	}
	...
	if !cfgMgr.IsLoadedFromJSON(config.Allocator, cfgs.BalancedAlloc.ConfigKey()) {
		logger.Debugf("when no allocator is specified, we assume 'freespace' 'storage'")
		cfgs.BalancedAlloc.AllocateBy = []string{"storage", "freespace"}
	}
