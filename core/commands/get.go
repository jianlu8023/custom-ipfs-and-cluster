...
import (
	...
	mfiles "github.com/ipfs/kubo/thirdparty/files"
)
...
`,
	...
	Options: []cmds.Option{
		...
		cmds.BoolOption(decryptGetOptionName, "d", "Decrypt file.").WithDefault(false),
	},
	...
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		logger.GetGetLogger().Debugf("starting get a file ...")

		ctx := req.Context

		decrypt, _ := req.Options[decryptGetOptionName].(bool)
		logger.GetGetLogger().Debugf("get file with decrypt %v", decrypt)
		...
		reader, err := fileArchive(file, p.String(), archive, cmplvl, decrypt)
		if err != nil {
			return err
		}
		go func() {
...
func fileArchive(f files.Node, name string, archive bool, compression int, decrypt bool) (io.ReadCloser, error) {
	logger.GetGetLogger().Debugf("fileArchive name %v archive %v compression %v decrypt %v", name, archive, compression, decrypt)
	...
	if !archive && compression != gzip.NoCompression {
		...
		if decrypt {
			reader, err := sm4.Decrypt(r)
			if err != nil {
				return nil, err
			}
			go func() {
				if _, err := io.Copy(maybeGzw, reader); checkErrAndClosePipe(err) {
					return
				}
		...
		// the case for 1. archive, and 2. not archived and not compressed,
		// in which tar is used anyway as a transport format

		// construct the tar writer

		if decrypt {
			w, err := mfiles.NewTarWriter(maybeGzw)
			if checkErrAndClosePipe(err) {
				return nil, err
			}
			go func() {
