...
`,
	...
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		logger.GetAddLogger().Debugf("starting add file to ipfs ...")
		...
		toadd := req.Files
		if wrap {
			logger.GetAddLogger().Debugf("this section wrap is true")
			// 将请求中的文件加入toadd中 ，toadd是一个files.Directory
		...
		logger.GetAddLogger().Debugf("get ipfsNode %v", ipfsNode.Identity.String())
		...
		for addit.Next() {
			...
			go func() {
				...
				// 调用add方法
				// pathAdded /ipfs/QmQkJVYPUKCD8L7QybxmYp9jw3rLJ8UGdp4fLLd9RwZu9G
