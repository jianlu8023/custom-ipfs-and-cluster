...
var CatCmd = &cmds.Command{
	...
	Options: []cmds.Option{
		...
		cmds.BoolOption(decryptCatOptionName, "d", "Decrypt file.").WithDefault(false),
	},
	Run: func(req *cmds.Request, res cmds.ResponseEmitter, env cmds.Environment) error {
		logger.GetCatLogger().Debugf("starting cat a file ...")
		...
		decrypt, _ := req.Options[decryptCatOptionName].(bool)

		logger.GetCatLogger().Debugf("cat file with decrypt %v", decrypt)
		err = req.ParseBodyArgs()
		if err != nil {
			return err
		}

		readers, length, err := cat(req.Context, api, req.Arguments, int64(offset), int64(max), decrypt)
		if err != nil {
			return err
		}
...
func cat(ctx context.Context, api iface.CoreAPI, paths []string, offset int64, max int64, decrypt bool) ([]io.Reader, uint64, error) {
	...
	for _, pString := range paths {
		...
		if max > 0 && length >= uint64(max) {
			logger.GetCatLogger().Debugf("cat a file with -o or -l")
			...
			if decrypt {
				...
				if err != nil {
					return readers, length, err
				}
				if overshoot := int64(length - uint64(max)); overshoot != 0 {
					r = io.LimitReader(reader, int64(size)-overshoot)
					length = uint64(max)
				}
			} else {
				if overshoot := int64(length - uint64(max)); overshoot != 0 {
